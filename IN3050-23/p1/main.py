...
if sys.platform != "linux":
	...
	print("/*")
	print("**")
	print("**", "Fancy output has been disabled. Use Linux for ANSI character support")
	print("**")
	print("*/")
	time.sleep(3)
...
class Salesman:
	...
	def __init__(self, city_coords, plot_size, max_cities=None, verbose=False, timings=True):
		...
		self.funcmap = {
				"Exhaustive Search": self.exhaustive_search,
	...
	def plot_figure(self):
		self.maps[self.current_axis] = Basemap(
	...
	def run_new_algorithm(self, plot_x, plot_y, algorithm, max_cities=None, cities_used=None):
		if not self.cities:
			self.seed_cities(max_cities, cities_used)

		self.current_axis = self.plots[plot_y][plot_x]
		self.current_axis.set_title(f"{algorithm} with {len(self.cities)} cities")

		self.plot_figure()  # empty canvas

		start_timer = timeit.default_timer()
		self.shortest, self.longest = self.funcmap[algorithm]()
		self.timer = timeit.default_timer() - start_timer
...
if __name__ == "__main__":
	...
	sample_sizes = 6, 10, 24

	ts = Salesman(
		data.cities_coords,
		plot_size=(len(sample_sizes), len(algos)),
		timings=False,
		verbose=False
	)

	timings = {}
	for sample_i, city_amount in enumerate(sample_sizes):
		ts.seed_cities(max_cities=city_amount)

		for algo_i, algorithm, in enumerate(algos):
			algotimes = []

			if algorithm == "Exhaustive Search" and city_amount > 12:
				continue

			for _ in range(20):
				ts.run_new_algorithm(algo_i, sample_i, algorithm)
				dist = ts.total_distance(ts.shortest)
				print(f"ALGORITHM {algorithm} including {city_amount} cities FINISHED after {dist} length units", end="\n\n")
				dp = dist, ts.get_timer()
				algotimes.append(dp)
			timings[(algorithm, city_amount)] = algotimes

	ts.start()

	fig, axs = plt.subplots(len(sample_sizes), len(algos))

	for algopair, datapair in timings.items():
		algo, city_n = algopair
		dists = [ dp[0] for dp in datapair ]
		times = [ dp[1] for dp in datapair ]

		m = (max(dists) - min(dists)) / (max(times) - min(times))
		b = min(dists)

		ts = np.array(times)
		ds = np.array(dists)
		m, b = np.polyfit(ts, ds, 1)
		...
		plt_x = algos.index(algo)
		plt_y = sample_sizes.index(city_n)
		axs[plt_x,plt_y].scatter(times, dists)
		axs[plt_x,plt_y].plot(ts, m*ts+b)
